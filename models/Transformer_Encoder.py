...
class EncoderLayer(nn.Module):
    ...
    """
    def __init__(self, d_model, num_heads, d_ff, dropout_r, act_fn_ecd):
        super(EncoderLayer, self).__init__()
        self.attention = MultiHeadAttention(d_model = d_model, num_heads = num_heads)
        self.norm1 = LayerNormalization(parameters_shape = [d_model])
        self.dropout = nn.Dropout(p = dropout_r)
        self.ffn = MLP(d_model = d_model, d_ff = d_ff, act_fn = act_fn_ecd, dropout_r = dropout_r)
...
# Feedforward MLP
class MLP(nn.Module):
    def __init__(self, d_model, d_ff, act_fn, dropout_r = 0.1):
